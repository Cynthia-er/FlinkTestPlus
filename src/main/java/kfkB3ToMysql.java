...
//        streamSource.print();
        //将流中的数据转化为对象
        SingleOutputStreamOperator<Bean3> bean3 = streamSource.map(s -> {
            String[] arr = s.split(",");
            return new Bean3(Integer.parseInt(arr[0]), Integer.parseInt(arr[1]));
        });

        //将类型为Bean1的流转成表
        tenv.createTemporaryView("bean3",bean3);

//        tenv.executeSql("select * from bean3").print();
        ...
        //将表中数据插入到MySQL
        tenv.executeSql("insert into t_mysql select * from bean3");
